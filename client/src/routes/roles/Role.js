...
class Role extends Component {
    ...
    delete () {
      ...
      axios.delete('/api/role/delete/' + this.state._id + '/' + RestaurantId, {token: localStorage.getItem('user_id') , RestaurantId: this.state.RestaurantId}).then((res) => {
       console.log(res)
        if (res.data.success) {    
          this.setState({
    ...
    dataListRender() {
      ...
      axios.get('/api/role/list/' + pageSize + '/' + currentPage +'/'+ orderBy + '/' + orderAs + '/'+ search + '/' + RestaurantId, {token: localStorage.getItem('user_id')}).then((res) => { 
        if (res.data.success) {
          var  totalPages = 0
      ...
      },
      {
        ...
        formatter: (cell, row, rowIndex, extraData) => (
         ...
         <div>
              ...
              <Link to={`/app/role/detail/${row._id}`} data-toggle="tooltip" title="Detail" ><i className='iconsmind-Information' /></Link>
                  <Link to={`/app/role/edit/${row._id}`} style={{'marginLeft': '15px'}} data-toggle="tooltip" title="Edit" ><i className='simple-icon-pencil' /></Link>
       ...
       :
        ...
        <div className="disable-text-selection">
            <Row>
              ...
              <label className="col-md-3" style={{'paddingRight':'0px', 'paddingLeft':'0px'}}><h5 style={{'marginTop':'.5rem'}}><b>Roles list</b></h5></label>  
            
              
                <Link className='btn btn btn-primary' style={{'float':'right' ,'marginBottom':'15px'}} color='primary' to={`/app/role/add`} >New Role<img className='img img-responsive' style={{'marginLeft':'10px','marginBottom':'5px', 'height':'22px'}} src='../../assets/img/hotel-white.png'></img></Link>
                    {"  "}    
            ...
            <Card>
              ...
              <CardBody style={{'padding':'0px'}}>
                <div id='table'>
                <BootstrapTable  keyField='_id'  remote={ { sort: true } } data={  roles }  columns={ columns } bordered={ true } noDataIndication="Table is Empty" onTableChange={this.changeOrderBy}   hover condensed   />
                </div>
                <div className='row'>
