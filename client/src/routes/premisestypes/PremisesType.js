...
class PremisesType extends Component {
    constructor(props) {
      super(props);
    ...
    delete () {
      ...
      axios.delete('/api/premisestype/delete/' + this.state._id + '/' + RestaurantId, {token: localStorage.getItem('user_id') , RestaurantId: this.state.RestaurantId}).then((res) => {
       console.log(res)
        if (res.data.success) {    
          this.setState({
    ...
    dataListRender() {
      ...
      axios.get('/api/premisestype/list/' + pageSize + '/' + currentPage +'/'+ orderBy + '/' + orderAs + '/'+ search + '/' + RestaurantId, {token: localStorage.getItem('user_id')}).then((res) => { 
        if (res.data.success) {
          var  totalPages = 0
      ...
      },
      {
        ...
        formatter: (cell, row, rowIndex, extraData) => (
         ...
         <div>
              ...
              <Link to={`/app/premisestype/detail/${row._id}`} data-toggle="tooltip" title="Detail" ><i className='iconsmind-Information' /></Link>
                  <Link to={`/app/premisestype/edit/${row._id}`} style={{'marginLeft': '15px'}} data-toggle="tooltip" title="Edit" ><i className='simple-icon-pencil' /></Link>
 ...
 var Premisestypes = this.state.Premisestypes
       ...
       :
        ...
        <div className="disable-text-selection">
            <Row>
              ...
              <label className="col-md-3" style={{'paddingRight':'0px', 'paddingLeft':'0px'}}><h5 style={{'marginTop':'.5rem'}}><b>Premises types list</b></h5></label>  
            
              
                <Link className='btn btn btn-primary' style={{'float':'right' ,'marginBottom':'15px'}} color='primary' to={`/app/premisestype/add`} >New Premises Type<img className='img img-responsive' style={{'marginLeft':'10px','marginBottom':'5px', 'height':'22px'}} src='../../assets/img/hotel-white.png'></img></Link>
                    {"  "}    
      ...
      <div className='col-md-12'>
        <Pagination style={{'lineHeight':'1.2', 'float':'right'}}
          ...
          <ModalHeader toggle={this.toggleDanger}>Delete Premises type</ModalHeader>
          <ModalBody>
                Are you sure to delete this Premises type?
                  </ModalBody>
          <ModalFooter>
  ...
  const mapStateToProps = ({  authUser }) => {
    ...
    export default withRouter(
      connect(
        mapStateToProps
      )(injectIntl(mouseTrap(PremisesType)))
    )
