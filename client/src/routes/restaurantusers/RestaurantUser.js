...
class RestaurantUser extends Component {
    ...
    changeOrderBy= (type, { sortField, sortOrder }) => {
    ...
    delete () {
      ...
      axios.delete('/api/restaurantusers/delete/' + this.state._id + '/' + RestaurantId, {token: localStorage.getItem('user_id') , RestaurantId: this.state.RestaurantId}).then((res) => {
       console.log(res)
        if (res.data.success) {    
          this.setState({
    ...
    dataListRender() {
      ...
      axios.get('/api/restaurantusers/list/' + pageSize + '/' + currentPage +'/'+ orderBy + '/' + orderAs + '/'+ search + '/' + RestaurantId, {token: localStorage.getItem('user_id')}).then((res) => { 
        if (res.data.success) {
          var  totalPages = 0
          debugger
      ...
      },
      {
        ...
        formatter: (cell, row, rowIndex) => (
         ...
         <div>
              ...
              <Link to={`/app/restaurantusers/detail/${row._id}`} data-toggle="tooltip" title="Detail" ><i className='iconsmind-Information' /></Link>
                  <Link to={`/app/restaurantusers/edit/${row._id}`} style={{'marginLeft': '15px'}} data-toggle="tooltip" title="Edit" ><i className='simple-icon-pencil' /></Link>
                  <Link to='#'onClick={() => this.toggle(row._id, rowIndex)} style={{'marginLeft': '15px'}} data-toggle="tooltip" title="Delete" ><i className='simple-icon-trash' /></Link>
         </div>  
      )
      }
      ];
 ...
 var RestaurantUsers = this.state.RestaurantUsers
       ...
       :
        ...
        <div className="disable-text-selection">
            <Row>
              ...
              <label className="col-md-3" style={{'paddingRight':'0px', 'paddingLeft':'0px'}}><h5 style={{'marginTop':'.5rem'}}><b>Restaurant users list</b></h5></label>  
            
              
                <Link className='btn btn btn-primary' style={{'float':'right' ,'marginBottom':'15px'}} color='primary' to={`/app/restaurantusers/add`} >New Restaurant User<img className='img img-responsive' style={{'marginLeft':'10px','marginBottom':'5px', 'height':'22px'}} src='../../assets/img/hotel-white.png'></img></Link>
                    {"  "}    
            ...
            <Card>
              ...
              <CardBody style={{'padding':'0px'}}>
                <div id='table'>
                <BootstrapTable  keyField='_id'  remote={ { sort: true } } data={  RestaurantUsers }  columns={ columns } bordered={ true } noDataIndication="Table is Empty" onTableChange={this.changeOrderBy}   hover condensed   />
                </div>
                <div className='row'>
      ...
      <div className='col-md-12'>
        <Pagination style={{'lineHeight':'1.2', 'float':'right'}}
          ...
          <ModalHeader toggle={this.toggleDanger}>Delete Restaurant User</ModalHeader>
          <ModalBody>
                Are you sure to delete this Restaurant User?
                  </ModalBody>
          <ModalFooter>
  ...
  const mapStateToProps = ({  authUser }) => {
    ...
    export default withRouter(
      connect(
        mapStateToProps
      )(injectIntl(mouseTrap(RestaurantUser)))
    )
