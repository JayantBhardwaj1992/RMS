...
class RestaurantPremises extends Component {
    constructor(props) {
      super(props);
    ...
    delete () {
      ...
      axios.delete('/api/restaurantpremises/delete/' + this.state._id + '/' + RestaurantId, {token: localStorage.getItem('user_id') , RestaurantId: this.state.RestaurantId}).then((res) => {
      debugger
        console.log(res)
        if (res.data.success) {    
          this.setState({
    ...
    dataListRender() {
      ...
      axios.get('/api/restaurantpremises/list/' + pageSize + '/' + currentPage +'/'+ orderBy + '/' + orderAs + '/'+ search + '/' + RestaurantId, {token: localStorage.getItem('user_id')}).then((res) => { 
      debugger
        if (res.data.success) {
          var  totalPages = 0
    ...
    render() {
      ...
      {
        dataField: 'RestaurantPremisesName',
        text: 'Premises Name',
        headerAlign: 'center',
        align: 'center',
      ...
      {
        dataField: 'PremisiveTypeId.PremisesType',
        text: 'PremisiveType',
        headerAlign: 'center',
        align: 'center',
      ...
      },
      {
        ...
        formatter: (cell, row, rowIndex, extraData) => (
         ...
         <div>
              ...
              <Link to={`/app/restaurantpremises/detail/${row._id}`} data-toggle="tooltip" title="Detail" ><i className='iconsmind-Information' /></Link>
                   <Link to={`/app/restaurantpremises/edit/${row._id}`} style={{'marginLeft': '15px'}} data-toggle="tooltip" title="Edit" ><i className='simple-icon-pencil' /></Link>
 ...
 var Premisestypes = this.state.Premisestypes
       ...
       :
        ...
        <div className="disable-text-selection">
            <Row>
              ...
              <label className="col-md-3" style={{'paddingRight':'0px', 'paddingLeft':'0px'}}><h5 style={{'marginTop':'.5rem'}}><b>Restaurant Premises list</b></h5></label>  
            
              
                <Link className='btn btn btn-primary' style={{'float':'right' ,'marginBottom':'15px'}} color='primary' to={`/app/restaurantpremises/add`} >New Restaurant Premises<img className='img img-responsive' style={{'marginLeft':'10px','marginBottom':'5px', 'height':'22px'}} src='../../assets/img/hotel-white.png'></img></Link>
                    {"  "}    
            ...
            <Card>
              ...
              <CardBody style={{'padding':'0px'}}>
                <div id='table'>
                <BootstrapTable  keyField='_id'  remote={ { sort: true } } data={  Premisestypes }  columns={ columns } bordered={ true } noDataIndication="Table is Empty" onTableChange={this.changeOrderBy}   hover condensed   />
                </div>
                <div className='row'>
      ...
      <div className='col-md-12'>
        <Pagination style={{'lineHeight':'1.2', 'float':'right'}}
          ...
          <ModalHeader toggle={this.toggleDanger}>Delete Restaurant Premises</ModalHeader>
          <ModalBody>
                Are you sure to delete this Restaurant Premises?
                  </ModalBody>
          <ModalFooter>
  ...
  const mapStateToProps = ({  authUser }) => {
    ...
    export default withRouter(
      connect(
        mapStateToProps
      )(injectIntl(mouseTrap(RestaurantPremises)))
    )
