...
class Tax extends Component {
    ...
    delete () {
      ...
      axios.delete('/api/tax/delete/' + this.state._id + '/' + RestaurantId, {token: localStorage.getItem('user_id') , RestaurantId: this.state.RestaurantId}).then((res) => {
 
      
        if (res.data.success) {       
          var array = [...this.state.Taxs]
          if (this.state.index !== -1) {
            array.splice(this.state.index, 1)
            this.setState({Taxs: array, modal: !this.state.modal })
          }
        } else {
          this.setState({
            isError: true,
    ...
    dataListRender() {
      ...
      axios.get('/api/tax/list/' + pageSize + '/' + currentPage +'/'+ orderBy + '/' + orderAs + '/'+ search + '/' + RestaurantId, {token: localStorage.getItem('user_id')}).then((res) => { 
        if (res.data.success) {
          var  totalPages = 0
    ...
    render() {
      const tax = this.state.Taxs.slice(0).reverse().map((res, index) => (

        <tr key={index}>
          <td>{index + 1 }</td>
          <td>{res.TaxName}</td>    
          <td>{res.TaxPercentage}</td>    
          <td>
            <Badge color={res.IsActive ? 'success' : 'danger'}>{res.IsActive ? 'Active' : 'InActive'}</Badge>
          </td>
          <td>
          <Link to={`/app/tax/detail/${res._id}` }  data-toggle="tooltip" title="Detail"><i className='iconsmind-Information' /></Link>
            <Link to={`/app/tax/edit/${res._id}`} style={{'marginLeft': '15px'}} data-toggle="tooltip" title="Edit" ><i className='simple-icon-pencil' /></Link>
            <Link to='#'onClick={e => this.toggle(res._id, index)} style={{'marginLeft': '15px'}} data-toggle="tooltip" title="Delete" ><i className='simple-icon-trash' /></Link>
          </td>
        </tr>
      ))
      ...
      },
      {
        ...
        formatter: (cell, row, rowIndex, extraData) => (
         ...
         <div>
              ...
              <Link to={`/app/tax/detail/${row._id}`} data-toggle="tooltip" title="Detail" ><i className='iconsmind-Information' /></Link>
                  <Link to={`/app/tax/edit/${row._id}`} style={{'marginLeft': '15px'}} data-toggle="tooltip" title="Edit" ><i className='simple-icon-pencil' /></Link>
       ...
       :
        <Fragment>
            ...
            <Row>
              ...
              <label className="col-md-3" style={{'paddingRight':'0px', 'paddingLeft':'0px'}}><h5 style={{'marginTop':'.5rem'}}><b>Tax list</b></h5></label>  
            
              
                <Link className='btn btn btn-primary' style={{'float':'right' ,'marginBottom':'15px'}} color='primary' to={`/app/tax/add`} >New Tax<img className='img img-responsive' style={{'marginLeft':'10px','marginBottom':'5px', 'height':'22px'}} src='../../assets/img/hotel-white.png'></img></Link>
                    {"  "}    
            ...
            <Card>
              ...
              <CardBody style={{'padding':'0px'}}>
                <div id='table'>
                <BootstrapTable  keyField='_id'  remote={ { sort: true } } data={  taxs }  columns={ columns } bordered={ true } noDataIndication="Table is Empty" onTableChange={this.changeOrderBy}   hover condensed   />
                </div>
                <div className='row'>
      ...
      <div className='col-md-12'>
        <Pagination style={{'lineHeight':'1.2', 'float':'right'}}
          ...
          <ModalHeader toggle={this.toggleDanger}>Delete Tax</ModalHeader>
          <ModalBody>
