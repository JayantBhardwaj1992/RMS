...
class Restaurant extends Component {
    ...
    delete () {
      ...
      axios.delete('/api/restaurants/delete/' + this.state._id, {token: localStorage.getItem('user_id')}).then((res) => {
    ...
    dataListRender() {
      ...
      const {selectedPageSize,currentPage,selectedOrderOption , selectedOrderField} = this.state;
      ...
      axios.get('/api/restaurants/list/' + pageSize + '/' + currentPage +'/'+ orderBy + '/' + orderAs + '/'+ search, {token: localStorage.getItem('user_id')}).then((res) => {
    ...
    render() {
      ...
      var row_n = startIndex+1
      const endIndex= (this.state.currentPage)*this.state.selectedPageSize
      const {messages} = this.props.intl;

     

      const restaurantst = this.state.Restaurants;
...
},
{
  ...
  formatter: (cell, row, rowIndex, extraData) => (
   ...
   <div>
        ...
        <Link to={`/app/restaurants/detail/${row._id}`} data-toggle="tooltip" title="Detail" ><i className='iconsmind-Information' /></Link>
            <Link to={`/app/restaurants/edit/${row._id}`} style={{'marginLeft': '15px'}} data-toggle="tooltip" title="Edit" ><i className='simple-icon-pencil' /></Link>
        ...
        <Fragment>
          <div className="disable-text-selection">
            <Row>
              ...
              <label className="col-md-3" style={{'paddingRight':'0px', 'paddingLeft':'0px'}}><h5 style={{'marginTop':'.5rem'}}><b>Restaurant list</b></h5></label>  
            
              
                <Link className='btn btn btn-primary' style={{'float':'right' ,'marginBottom':'15px'}} color='primary' to={`/app/restaurants/add`} >New Restaurant<img className='img img-responsive' style={{'marginLeft':'10px','marginBottom':'5px', 'height':'22px'}} src='../../assets/img/hotel-white.png'></img></Link>
                    {"  "}    
            ...
            <Card>
              ...
              <CardBody style={{'padding':'0px'}}>
                <div id='table'>
                <BootstrapTable  keyField='_id'  remote={ { sort: true } } data={  restaurantst }  columns={ columns } bordered={ true } noDataIndication="Table is Empty" onTableChange={this.changeOrderBy}   hover condensed   />
                </div>
                <div className='row'>
      ...
      <div className='col-md-12'>
        <Pagination style={{'lineHeight':'1.2', 'float':'right'}}
          ...
          <ModalHeader toggle={this.toggleDanger}>Delete Restaurant</ModalHeader>
          <ModalBody>
  ...
  export default injectIntl(mouseTrap(Restaurant))
